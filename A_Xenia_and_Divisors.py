...
def solve():
    ...
    ans_arr = []
    for i in range(0, bp):
        # print(arr[i], arr[i + bp], arr[i + 2 * bp])
        # print(arr[i + 2 * bp] % arr[i])
        if (
            arr[i + bp] <= arr[i]
            or arr[i + bp] % arr[i] != 0
            or arr[i + 2 * bp] <= arr[i]
            or arr[i + 2 * bp] % arr[i] != 0
            or arr[i + bp] >= arr[i + 2 * bp]
            or arr[i + 2 * bp] % arr[i + bp] != 0
        ):
            ans = False
            break
        else:
            ans_arr.append([arr[i], arr[i + bp], arr[i + 2 * bp]])
    # print(ans)
    if not ans:
        print(-1)
    else:
        for a, b, c in ans_arr:
            print(a, b, c)
    # print(ans)
