...
def gcd(first, second):
    ...
    return first
def lcm(first, second): return first // gcd(first, second) * second
def ceil_div(numerator, denominator): return (numerator + denominator - 1) // denominator
...
def suffix_sum(arr):
    size = len(arr)
    suffix = [0]*(size+1)
    for index in range(size-1,-1,-1): suffix[index] = suffix[index+1]+arr[index]
    return suffix
...
def solve():
    ...
    max_quality = laptops[0][1]

    for i in range(1, n):
        if laptops[i][1] < max_quality:
            print("Happy Alex")
            return
        max_quality = max(max_quality, laptops[i][1])
