...
def solve():
    ...
    remainder = max_val % 5
    amp_val = max_val - remainder

    ans = (amp_val // 5) * min_val

    # handle last remainder block properly
    for val in range(1, min_val + 1):
        x_rem = val % 5
        needed = (5 - x_rem) % 5

        # check if needed remainder exists in last block
        if needed != 0 and needed <= remainder:
            ans += 1
