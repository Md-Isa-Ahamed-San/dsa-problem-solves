    ...
    for _ in range(t):
        ...
        for val in arr:
            if is_even(val):
                ...
                prefix_sum_arr.append(prefix_sum)
        for _ in range(q):
            left, right, value = map(int, input().strip().split())
            val = 0
            if is_even(value):
                val = 2
            else:
            ...
            # print(prefix_sum_arr[left-2])
            # print((val*(right-left+1)))
            # print(prefix_sum_arr[n-1])
            # print(prefix_sum_arr[n-1]-prefix_sum_arr[right-1])
            
            ans = ((val * (right - left + 1))
                + (prefix_sum_arr[n - 1]
                - prefix_sum_arr[right - 1])
            )
            if left-2>=0:
                ans+=prefix_sum_arr[left - 2]
            if is_even(ans):
                no()
            else:
                yes()
