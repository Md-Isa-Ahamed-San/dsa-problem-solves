...
# ---------- SOLVE ----------
def solve():
    total_len, a, b, c = map(int, input().strip().split())
    ans=-1
    for i in range(total_len//a+1):
        for j in range(total_len//b+1):
            reminder = total_len - (i*a)-(j*b)
            if reminder<0: # beshi niye felchi
                break
            if reminder%c==0:
                ans = max(ans,i+j+ (reminder//c))
    print(ans)
