...
def solve():
    ...
    for _ in range(t):
        ...
        for i in range(n): # max is in the last pos
            ans = max(ans, arr[-1] - arr[i])
        for i in range(n - 1): #max is in the middle and after the max we get the value which is largest diff 
            ans = max(ans, arr[i] - arr[i + 1])
        for i in range(1, n): # min in the first and max can be anywhere.
