...
def solve():
    ...
    for _ in range(t):
        ...
        if row <= col:  # vertical y axis
            if is_odd(col):
                # col odd hole start from col*col and then value decrease
                base = pow(col, 2)
                decrease = row - 1
                ans = base - decrease
                print(ans)
            else:  # col even hole start from (col-1)^2 +1 and then increase
                base = pow(col - 1, 2) + 1
                increase = row - 1
                ans = base + increase
                print(ans)
        else:  # row>col
            if is_even(row):  # row even hole start from row^2 and decrease
                base = pow(row, 2)
                decrease = col - 1
                ans = base - decrease
                print(ans)
            else:  # row odd hole start from (row-1)^2 +1 and increase
                base = pow(row - 1, 2) + 1
                increase = col - 1
                ans = base + increase
                print(ans)
