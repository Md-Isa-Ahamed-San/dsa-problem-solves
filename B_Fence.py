...
# ---------- SOLVE ----------
def solve():
    n, piano_len = map(int, input().strip().split())
    ...
    for val in arr:
        s += val
        prefix_sum.append(s)
    min_fence = prefix_sum[piano_len - 1]
    # print(prefix_sum)
    for i in range(piano_len, n, 1):
        val = prefix_sum[i] - prefix_sum[i - piano_len]
        if val < min_fence:
            min_fence = val
            idx = i
    if idx == 0:
