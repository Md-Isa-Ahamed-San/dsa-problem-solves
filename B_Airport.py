...
# ---------- SOLVE ----------
def solve():
    passenger, plane_number = map(int, input().strip().split())
    arr = list_ints()
    # print(passenger,plane_number,arr)
    arr1 = sorted(arr[:])
    minimum_money = 0
    passenger_for_max = passenger
    passenger_for_min = passenger
    for i in range(plane_number):
        while arr1[i] > 0:
            minimum_money += arr1[i]
            arr1[i] -= 1
            passenger_for_min -= 1
            if passenger_for_min == 0:
                break
        if passenger_for_min == 0:
            break
    max_arr = arr[:]
    ...
    while passenger_for_max > 0:
        max_arr.sort(reverse=True)
        max_money += max_arr[0]
        max_arr[0] -= 1
        passenger_for_max -= 1
        if passenger_for_max == 0:
            break
    print(max_money, minimum_money)
