...
def solve():
    ...
    left = sorted(ticket[:n])
    right = sorted(ticket[n:])

    all_less    = all(left[i] < right[i] for i in range(n))
    all_greater = all(left[i] > right[i] for i in range(n))

    print("YES" if all_less or all_greater else "NO")
