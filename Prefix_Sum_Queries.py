...
# ---------- SOLVE ----------
def calculate_sub_array_sum(arr):
    ans = []
    count = 0
...
def solve():
    ...
    arr = list_ints()
    for _ in range(q):
        opt, a, b = map(int, input().strip().split())
        if opt == 2:  # calculate max sub array sum between [a,b]
            ans_arr = calculate_sub_array_sum(arr)
            print(ans_arr)
            print(max(ans_arr[a-1:b])-ans_arr[a-2])
